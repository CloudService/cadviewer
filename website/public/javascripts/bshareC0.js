(function () {
	function C() {
		...
		for (b in d.pnMap)
			a = d.topMap[b], g += ".bshare-custom .bshare-" + b + '{background:url("' + p + (a ? l : b) + q + '")' + (a ? " no-repeat 0 " + a * D + "px;" + c : m) + ".bshare-custom.icon-medium .bshare-" + b + '{background:url("' + r + (a ? l : b) + '.gif")' + (a ? " no-repeat 0 " + a * E + "px;" + c : m) + ".bshare-custom.icon-medium-plus .bshare-" + b + '{background:url("' + s + (a ? l : b) + '.gif")' + (a ? " no-repeat 0 " + a * F + "px;" + c : m) + ".bshare-custom.icon-large .bshare-" +
		...
		for (b in y)
			e = H + y[b], g += ".bshare-custom .bshare-more." + e + "{background:url(" + p + e + q + ") no-repeat;}.bshare-custom.icon-medium a.bshare-more." + e + "{background:url(" + r + e + ".gif) no-repeat;}.bshare-custom.icon-medium-plus a.bshare-more." + e + "{background:url(" + s + e + ".gif) no-repeat;}.bshare-custom.icon-large a.bshare-more." + e + "{background:url(" + t + e + ".gif) no-repeat;}";
		...
		if (!d.anchorsBinded) {
			...
			for (f = 0; f < B.length; f++) {
				...
				for (i = 0; i < x.length; i++)
					o = x[i], o.buttonIndex = f, A.addEventListener ? o.addEventListener("click", g, !1) : o.attachEvent("onclick", g)
			}
		}
	}
	var w = window,
	...
	j.ready(function () {
		var a = function () {
			d.completed ? (C(), d.buttonType !== 15 && n.pop !== -1 && !n.beta && !n.popjs && j.loadScript(d.jsBasePath + "styles/bshareS887.js?v=20130401")) : setTimeout(a, 50)
		};
		a()
	})
})();
